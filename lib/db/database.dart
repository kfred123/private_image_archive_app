...
class DataBaseFactory {
  static Future<DataBaseConnection> connect() async {
    ...
    Database db = await openDatabase(
        join(dbPath, "privateimagearchive.db"),
        onCreate: DataBaseFactory.onCreate,
        version: 1
    );
...
class DataBaseConnection {
  ...
  Future<List<dynamic>> query<T>(dynamic Function() constructor,
      ...
      for(var row in queryResult) {
        ...
        for(DbColumn dbColumn in obj.getColumns()) {
          if(row.containsKey(dbColumn.name)) {
            ...
            Logging.logError("databaserow does not contain a value for column ${dbColumn.name}, id=${obj.id}");
          }
        }
      }
    }
